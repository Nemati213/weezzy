...
public class EmailVerificationTokenCodec {
	...
	public Optional<ParsedEmailVerificationToken> parse(String rawToken) {
		if (rawToken == null) {
		...
		try {
			...
			return Optional.of(new ParsedEmailVerificationToken(
					tokenId,
					hash(secret)
			));
	...
	public record ParsedEmailVerificationToken(
			UUID id,
			String secretHash
	) {
	}
