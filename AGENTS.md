...
5. Добавить matching/lifecycle workers:
   - запуск matching раз в минуту и атомарный claim целой корзины;
   - перевод несматченных заявок в `EXTENSION_REQUESTED`;
   - таймаут предложения, `EXPIRED`, максимум продлений и завершение старых групп;
   - идемпотентность, retry и безопасная параллельная обработка.
