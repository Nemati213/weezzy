...
- Функциональных backend-задач по текущему MVP экспресс-обедов не осталось.
- Кнопка «Хочу остаться на связи» реализуется на клиенте через существующий
  `POST /api/votes/{profileId}` с действием `LIKE`; `PASS` в интерфейсе группы можно
  не показывать.
- После изменений инфраструктуры ланчей запускать полный Testcontainers suite.
