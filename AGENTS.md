...
- matching worker запускается раз в минуту, группирует `SEARCHING` заявки по
  жёстким критериям `location_id + time_slot` и транзакционно claim-ит корзину через
  PostgreSQL advisory lock;
- после claim worker блокирует все актуальные заявки корзины через
  `SELECT ... FOR UPDATE` в детерминированном порядке; построчный `SKIP LOCKED`
  здесь не использовать, потому что он может разделить одну корзину между workers;
...
- в MVP пользователь может отменить заявку до формирования группы, но после
  `MATCHED` не может отказаться от конкретной группы и повторно искать компанию в
  тот же день; если группа отменена системой, участников не штрафовать и по
  возможности вернуть их заявки в `SEARCHING` либо предложить продление;
...
Этот план является рекомендуемой декомпозицией, а не обязательной архитектурой.
Другой агент вправе предложить иной порядок или дизайн, если он сохраняет описанные
выше бизнес-инварианты, транзакционность, безопасность и тестируемость.

1. Базовые каталоги и заявки — реализовано:
   - университеты и активные локации;
   - `LunchRequest`, конфигурируемые окно, округление и продления;
   - API создания, чтения, отмены и принятия продления;
   - запрет повторной заявки после `MATCHED` в тот же календарный день.
2. Модель сформированной группы — реализовано:
   - `LunchGroup`, `LunchGroupMember` и их статусы;
   - связь участника с исходной заявкой;
   - уникальность участника внутри группы и запрет повторного использования заявки;
   - repository-запросы активной группы и состава участников.
3. `LunchGroupFormationService` — реализовано:
   - в одной транзакции создать группу и 2–4 members;
   - повторно проверить eligibility и попарные blocks;
   - перевести заявки в `MATCHED` и не допустить повторного использования;
   - подготовить запись outbox-события без прямой отправки уведомления.
4. Matching pipeline — реализовано:
   - общий контракт стратегии и объект корзины `location_id + time_slot`;
   - strict topic, mixed topic и desperate pair;
   - детерминированный порядок и максимизация числа сматченных пользователей;
   - передача следующей стратегии только оставшихся заявок.
5. Matching worker и lifecycle заявок — реализовано:
   - matching запускается раз в минуту и атомарно claim-ит целую корзину;
   - параллельная обработка, retry и повторный запуск безопасны;
   - due-заявки переводятся в `EXTENSION_REQUESTED` через конкурентный worker;
   - offer ID, deadline, target slot и outbox-событие создаются транзакционно;
   - офферы без ответа и заявки без доступного продления переходят в `EXPIRED`;
   - принятие продления и истечение конкурентно безопасны и идемпотентны.
6. Transactional outbox для формирования и отмены группы — реализовано:
   - событие системной отмены группы;
   - notification types, payloads и handlers с идемпотентной доставкой.
7. API и lifecycle группы — реализовано:
   - получение текущей группы и её участников;
   - доступ только участникам;
   - автоматический переход `ACTIVE` в `COMPLETED` после настраиваемой длительности;
   - системная отмена несовместимых и невалидных групп до начала обеда;
   - освобождение membership и возврат eligible-заявок в `SEARCHING`;
   - отсутствие пользовательского rematch после `MATCHED` в MVP.
8. Добавить временный REST-чат:
   - сообщения, отправка и cursor pagination;
   - проверка активного membership;
   - закрытие после завершения группы и retention cleanup.
9. Добавить «Хочу остаться на связи»:
   - отдельные взаимные выборы внутри lunch-группы;
   - раскрытие Telegram только после взаимности;
   - отдельное outbox-событие, без создания обычного `ProfileMatch`.
10. Провести финальное укрепление:
    - интеграционные тесты API, lifecycle, блокировок и санкций;
    - конкурентные тесты formation и workers;
    - тесты временных границ, деградации, retry и идемпотентности;
    - полный тестовый прогон и необходимые метрики/наблюдаемость.
